...
```
```diff
! Странно смотрится условие в миграции. Можно добавить в таблицу признак локали и добавлять все приветствия?
```
...
! Когда в классе будет больше одного метода, возможно пригодится отдельный метод для подстановки переменных в шаблон.
```
...
! Почему getRandomGreetingMessage — статический?
```
